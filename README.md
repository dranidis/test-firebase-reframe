...
* [`src/test_firebase/`](src/test_firebase/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/test_firebase/core.cljs): contains the SPA entry point, `init`
* [`test/test_firebase/`](test/test_firebase/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `test-firebase` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/test_firebase/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns test-firebase.example
  (:require [test-firebase.config :as config])
